  ...
  app.controller('twxConnectorCtrl', function($scope, $rootScope, DataConnector, $timeout, $log, filterService) {
    ...
    this.addDataBinding = function(fromExpression, toProperty, elementToUpdate, sourceName) {
      if (fromExpression && toProperty) {
        ...
        if (sourceEl.length === 1) {
          ...
          watchScope = angular.element(sourceEl[0]).scope();
        }
    ...
    this.addEventBinding = function(attrs, eventBindingElem) {
      ...
      if (eventSourceType && eventSourceType === 'event') {
        ...
        var eventBinding = angular.element(eventBindingElem);
        eventBinding.on('twxEventEmitted', function(event) {
          data = event.detail;
        ...
        // otherwise, must be a widget or service event binding.  Add an event listener to the source element.
        var sourceName = attrs.srcName;
        var query = (eventSourceType === 'widget') ? 'twx-widget[widget-id="' + sourceName + '"]' : 'twx-model[name="' + sourceName + '"]';
        var sourceElement = document.querySelector(query);

        if (sourceElement) {
          var eventSourceScope = angular.element(sourceElement).scope();

          // add a listener to the source element, listen to the event
          if (eventSourceScope) {
            eventSourceScope.$on(eventName, function(e) {
              addEventData(eventBindingElem, data);
              $rootScope.$broadcast(handler, data);
            });
          }
        } else {
          $log.error('Not able to locate the source element', sourceName, ', it will not get notified when the ', eventName, 'event occurs');
        }
      }
    };
  });
    ...
    return {
      ...
      link: function(scope, element, attrs, twxConnectorCtrl) {
        ...
        var handleSuccess = scope._successHandler = function(logicalName, data) {
          ...
          scope.$emit(logicalName + '-complete', {data: modelData, online: true});
        };
        var handleFailure = function(logicalName, reason, serviceParams) {
          setUpdating(false);
          scope.$emit(logicalName + '-failed', {service: logicalName, params: serviceParams, reason: reason});
        };
        var executeService = function(logicalName, eventParams) {
          ...
          twxConnectorCtrl.executeRegisteredService(logicalName, eventParams).then(function(data) {
            ...
            $rootScope.$broadcast('serviceFailure', {message: reason, service: logicalName});
            scope.$emit(logicalName + '-end');
          });
        };
    ...
    return {
      ...
      link: function(scope, element, attrs, twxConnectorCtrl) {
        ...
        var twxEventHandler = scope._twxEventHandler = function (data) {
          ...
          var eventBindings = document.querySelectorAll('twx-eventbind[src-name="' + modelName + '"][src-event="' + eventId + '"]');
          var twxEvent = new CustomEvent('twxEventEmitted', {'detail': data});
    ...
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        if(attrs.expression) {
          ...
          appScope.$on(eventName, createEventHandler(appScope, attrs.expression));
        }

        if(attrs.voiceresponse) {
          // register a handler for the 'voice response' when the app event is emitted
          appScope.$on(eventName, createEventHandler(appScope, 'app.speech.say("' + attrs.voiceresponse + '");'));
        }

        $log.debug('registering listeners for the app event', eventName);
        appScope.$on(eventName, function(event, data) {
          // if there are any event bindings bound to this event, notify the handler via a CustomEvent
          var eventBindings = document.querySelectorAll('twx-eventbind[src-event="' + eventName + '"][src-name="app"]');
          var twxEvent = new CustomEvent('twxEventEmitted', {'detail': data});
